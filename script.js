k = `R0eukfb7HV9aF3GtRfz2Mi1xiEZD2tKm`;
...
function handleGetCountries() {
    $.ajax({
        url: `https://calendarific.com/api/v2/countries?&api_key=R0eukfb7HV9aF3GtRfz2Mi1xiEZD2tKm`
    }).then(
        (data) => {
...
function handleGetHolidays(evt){
    ...
    if (userCountry.value === 'Please Choose a Country') {
        ...
        $.ajax({
            url: `https://calendarific.com/api/v2/holidays?&api_key=R0eukfb7HV9aF3GtRfz2Mi1xiEZD2tKm&country=${userCountry.value}&year=${userYear.value}&month=${userMonth.value}&day=${userDay.value}`
        }).then(
            (data) => {
